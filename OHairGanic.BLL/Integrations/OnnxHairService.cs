    ...
    public class OnnxHairService : IHairAnalysisService
    {
        ...
        public OnnxHairService()
        {
            ...
            if (!File.Exists(labelsPath))
                throw new FileNotFoundException("Không tìm thấy file labels.txt!", labelsPath);

            _session = new InferenceSession(modelPath);
            _labels = File.ReadAllLines(labelsPath);
        }

        // Phân tích ảnh từ file upload (giữ nguyên)
        public async Task<HairAnalyzeResponse> AnalyzeImageAsync(Stream imageStream)
        {
            return await ProcessImageAsync(imageStream);
        }

        // Phân tích ảnh từ URL Cloudinary
